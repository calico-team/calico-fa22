...
class Template {
	...
	*/
	static void solve (int N, int[] C) {
		...
		// Sorted list of bottles
		ArrayList<Integer> bottleOrder = new ArrayList<>();
		...
		// Which people will move?
		ArrayList<Integer> movedPeople = new ArrayList<>();
		// The sorted ordering of the moved people 
		ArrayList<Integer> movedPeopleOrdering = new ArrayList<>();
		...
		Collections.sort(
			movedPeopleOrdering, Comparator.comparingInt(x -> C[x])
		);
		for (int i = 0; i < movedPeople.size(); i++)
			res[movedPeople.get(i)] = movedPeopleOrdering.get(i);
		
		out.println(ans);
		for (int i = 0; i < N; i++)
			out.print((res[i] + 1) + (i == N - 1 ? "\n" : " "));
	}
